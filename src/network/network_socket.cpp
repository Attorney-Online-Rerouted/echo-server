    ...
    : QObject(parent)
{
  ...
  connect(m_client_socket.tcp, &QTcpSocket::readyRead, this, &NetworkSocket::readData);
  connect(m_client_socket.tcp, &QTcpSocket::disconnected, this, &NetworkSocket::clientDisconnected);
    ...
    : QObject(parent)
{
  ...
  connect(m_client_socket.ws, &QWebSocket::textMessageReceived, this, &NetworkSocket::ws_readData);
  connect(m_client_socket.ws, &QWebSocket::disconnected, this, &NetworkSocket::clientDisconnected);

  bool l_is_local = (m_client_socket.ws->peerAddress() == QHostAddress::LocalHost) || (m_client_socket.ws->peerAddress() == QHostAddress::LocalHostIPv6) || (m_client_socket.ws->peerAddress() == QHostAddress("::ffff:127.0.0.1"));
...
{
  if (m_client_socket.tcp->bytesAvailable() > 30720)
  { // Client can send a max of 30KB to the server.
