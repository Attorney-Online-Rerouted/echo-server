...
{
  PacketInfo info{.acl_permission = ACLRole::Permission::NONE, .min_args = 2, .header = "MC"};
  return info;
}
...
void PacketMC::handlePacket(AreaData *area, AOClient &client) const
{
  ...
  if (client.getServer()->getMusicList().contains(l_argument) || client.m_music_manager->isCustom(client.m_current_area, l_argument) || l_argument == "~stop.mp3")
